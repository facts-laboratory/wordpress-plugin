...
class LevelDbCache {
    ...
    async get(cacheKey, returnDeepCopy) {
        const contractCache = this.db.sublevel(cacheKey.key, { valueEncoding: 'json' });
    ...
    async getLast(key) {
        const contractCache = this.db.sublevel(key, { valueEncoding: 'json' });
        // manually opening to fix https://github.com/Level/level/issues/221
        await contractCache.open();
    ...
    async put(stateCacheKey, value) {
        const contractCache = this.db.sublevel(stateCacheKey.key, { valueEncoding: 'json' });
        // manually opening to fix https://github.com/Level/level/issues/221
        await contractCache.open();
