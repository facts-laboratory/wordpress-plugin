...
class HandlerBasedContract {
    ...
    async viewStateForTx(input, interactionTx) {
        this.logger.info(`View state for ${this._contractTxId}`);
        return await this.doApplyInputOnTx(input, interactionTx);
    }
    ...
    async applyInput(input, transaction) {
        ...
        return await this.doApplyInputOnTx(input, transaction);
    }
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false, reward) {
        ...
        if (!this._evaluationOptions.internalWrites && strict) {
            ...
            const caller = this.signature.type == 'arweave'
    ...
    async createExecutionContext(contractTxId, upToSortKey, forceDefinitionLoad = false, interactions) {
        ...
        if (contractDefinition) {
            ...
            handler = (await this.warp.executorFactory.create(contractDefinition, contractEvaluationOptions, this.warp));
        }
        return {
            warp: this.warp,
            contract: this,
    ...
    async doApplyInputOnTx(input, interactionTx) {
        this.maybeResetRootContract();
        let evalStateResult;
        ...
        if (!this.isRoot() && this.hasUncommittedState(this.txId())) {
            evalStateResult = {
                ...
                cachedValue: this.getUncommittedState(this.txId())
            };
        }
        else {
